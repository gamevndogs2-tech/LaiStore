...
<div class="mb-6 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    <div>
        ...
        <p class="text-slate-500 text-xs sm:text-sm mt-1">Tiếp nhận đơn hàng mới và kiểm tra hình thức thu tiền / thanh toán.</p>
    </div>
...
<!-- BẢNG ĐIỀU KHIỂN SHIPPER (CUỘN NGANG TRÊN MOBILE) -->
        ...
        <table class="w-full text-left border-collapse min-w-[750px]">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100 text-slate-400 uppercase text-[10px] sm:text-xs font-bold">
                    ...
                    <th class="p-3.5 sm:p-4">Khách Hàng</th>
                    <th class="p-3.5 sm:p-4">Địa Chỉ & SĐT</th>
                    <th class="p-3.5 sm:p-4">Thanh Toán</th>
                    <th class="p-3.5 sm:p-4">Tiền Thu (COD)</th>
            ...
            <tbody class="divide-y divide-slate-100 text-xs sm:text-sm font-medium">
                    ...
                    <?php while ($o = $orders->fetch_assoc()): ?>
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="p-3.5 sm:p-4">
                                ...
                                <div class="text-[11px] text-slate-500 mt-0.5"><i class="fa-solid fa-phone text-slate-400 mr-1"></i><?= htmlspecialchars($o['phone']) ?></div>
                            </td>
                            
                            <td class="p-3.5 sm:p-4">
                                <?php if ($o['payment_method'] === 'WALLET'): ?>
                                    <span class="bg-indigo-50 text-indigo-600 border border-indigo-200/60 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-extrabold inline-flex items-center gap-1">
                                        <i class="fa-solid fa-wallet"></i> Ví Wallet
                                    </span>
                                <?php else: ?>
                                    <span class="bg-amber-50 text-amber-600 border border-amber-200/60 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-extrabold inline-flex items-center gap-1">
                            ...
                            <td class="p-3.5 sm:p-4 whitespace-nowrap">
                                <?php if ($o['payment_method'] === 'WALLET'): ?>
                                    <span class="text-[11px] sm:text-xs font-extrabold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200/60">
                                        0 đ (Đã thanh toán)
                                    </span>
                                <?php else: ?>
                            ...
                            <td class="p-3.5 sm:p-4">
                                <?php if ($o['status'] === 'PENDING'): ?>
                                    <span class="bg-amber-50 text-amber-600 border border-amber-200/60 px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-bold inline-flex items-center gap-1 whitespace-nowrap"><i class="fa-solid fa-clock"></i> Chờ Nhận</span>
                            ...
                            <td class="p-3.5 sm:p-4 text-center">
                                <?php if ($o['status'] === 'PENDING'): ?>
                                    <a href="shipper.php?id=<?= $o['id'] ?>&update_status=SHIPPING" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3.5 py-2 rounded-xl text-[11px] sm:text-xs font-bold shadow-md shadow-indigo-100 transition inline-flex items-center gap-1 whitespace-nowrap">
                                        <i class="fa-solid fa-hand-holding-hand"></i> Nhận Đơn
                                    </a>
                                <?php elseif ($o['status'] === 'SHIPPING'): ?>
                                    <a href="shipper.php?id=<?= $o['id'] ?>&update_status=DELIVERED" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-2 rounded-xl text-[11px] sm:text-xs font-bold shadow-md shadow-emerald-100 transition inline-flex items-center gap-1 whitespace-nowrap">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="p-8 text-center text-slate-400 text-xs sm:text-sm">Hiện tại chưa có đơn hàng nào trong hệ thống.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
