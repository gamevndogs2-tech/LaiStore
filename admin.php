...
<div class="mb-8 flex items-center justify-between">
    <div>
        <div class="flex items-center gap-2">
            ...
            <h2 class="text-3xl font-extrabold text-slate-900">Bảng Điều Khiển Admin</h2>
        </div>
        <p class="text-slate-500 text-sm mt-1">Quản lý tài khoản, cộng tiền ví LaiStore Wallet và phân quyền hệ thống.</p>
    </div>
    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-slate-50 border-b border-slate-100 text-slate-400 uppercase text-xs font-bold">
                ...
                <th class="p-4">Số Dư Ví</th>
                <th class="p-4">Thẻ / Ngân Hàng</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="p-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick='openAddBalanceModal(<?= json_encode($u) ?>)' class="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1">
                                    <i class="fa-solid fa-circle-plus"></i> Cộng Tiền
                                </button>
                                <button onclick='openEditModal(<?= json_encode($u) ?>)' class="bg-amber-50 text-amber-600 hover:bg-amber-100 px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1">
                                    <i class="fa-solid fa-user-pen"></i> Sửa
                                </button>
                                <a href="javascript:void(0)" onclick="confirmDeleteUser(<?= $u['id'] ?>)" class="bg-rose-50 text-rose-600 hover:bg-rose-100 px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1">
...
<!-- MODAL CỘNG TIỀN VÍ -->
<div id="balanceModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-slate-900">Cộng Tiền Vào Ví Tài Khoản</h3>
        ...
        <form method="POST" action="admin.php" class="space-y-4">
            ...
            <div class="p-3 bg-slate-50 rounded-2xl border border-slate-100 mb-4">
                <div class="text-xs text-slate-400 uppercase font-bold">Tài khoản thụ hưởng</div>
                <div id="bal_username" class="text-base font-extrabold text-slate-800"></div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Số Tiền Cộng Thêm (VNĐ)</label>
                <input type="number" name="amount" min="1000" step="1000" required placeholder="100000" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-600 text-sm font-bold">
            </div>

            <div class="flex gap-3 pt-4">
                <button type="button" onclick="closeAddBalanceModal()" class="flex-1 bg-slate-100 text-slate-600 font-bold py-3 rounded-xl text-sm">Hủy</button>
                <button type="submit" name="add_balance_action" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-sm shadow-lg shadow-emerald-200">Cộng Tiền</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-slate-900">Chỉnh Sửa Tài Khoản</h3>
        ...
        <form method="POST" action="admin.php" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="username" id="edit_username" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-600 text-sm font-medium">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Số Điện Thoại</label>
                    <input type="text" name="phone" id="edit_phone" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-600 text-sm font-medium">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Địa Chỉ Email</label>
                <input type="email" name="email" id="edit_email" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-600 text-sm font-medium">
            </div>
            ...
            <div>
                ...
                <input type="password" name="new_password" placeholder="••••••••" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-600 text-sm font-medium">
            </div>

            <div class="flex gap-3 pt-4">
                <button type="button" onclick="closeEditModal()" class="flex-1 bg-slate-100 text-slate-600 font-bold py-3 rounded-xl text-sm">Hủy</button>
                <button type="submit" name="update_user" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl text-sm shadow-lg shadow-indigo-200">Lưu Thay Đổi</button>
            </div>
        </form>
    </div>
</div>
